...
<div @mouseover.away="open = false" class="relative" x-data="{ open: false }">
    <button @mouseover="open = !open"  :class="{'text-primary bg-white md:text-white md:bg-primary': open, 'text-primary bg-white': !open}" class="inline-grid items-center grid-flow-col cursor-pointer text-primary px-6 py-2 md:hover:text-white md:hover:bg-primary">
      {{ $trigger }}
